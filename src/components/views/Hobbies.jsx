    ...
    <section className="flex flex-col border border-green-700 h-screen md:flex-row min-h-[60vh] bg-[#0a0a0a] font-mono text-green-400 rounded-md overflow-hidden">
      ...
      <div className="flex-1 p-6 overflow-auto">
        <div className="mb-4">
          ...
          <p className="text-green-600 text-sm leading-relaxed whitespace-pre-line mt-2 mb-4">{activeHobby.description}</p>
        </div>
        ...
        {activeHobby.name === "Music" && (
          <div className="bg-[#111] p-4 rounded-md text-green-300 text-xs space-y-2 mb-4">
            ...
            <ul className="list-disc pl-6 space-y-1">
              <li>
                <a href="https://soundcloud.com/abhishek-chouhan-762644418" target="_blank" className="text-green-300 hover:text-green-100 underline">SoundCloud</a>
              </li>
              <li>
                <a href="https://music.youtube.com/channel/UCyOvS6dgGxielWP-l7O_TbQ" target="_blank" className="text-green-300 hover:text-green-100 underline">YouTube Music</a>
              </li>
            </ul>
          </div>
        )}
        ...
        {activeHobby.name === "Gaming" && activeHobby.stats && (
          <div className="bg-[#111] p-4 rounded-md text-green-300 text-xs space-y-3 mb-4">
            <p>Rating: {activeHobby.stats.rating} | Peak: {activeHobby.stats.peak} | K/D: {activeHobby.stats.kda} | Win Rate: {activeHobby.stats.winRate}</p>

            <div>
              <p className="font-semibold text-green-400">Top Agents:</p>
              <ul className="list-disc pl-6 space-y-1">
                {activeHobby.stats.topAgents.map((agent, idx) => (
                  <li key={idx}>{agent.name} - Matches: {agent.matches}, Win: {agent.win}, KDA: {agent.kda}</li>
                ))}
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="list-disc pl-6 space-y-1">
                {activeHobby.stats.topWeapons.map((weapon, idx) => (
                  <li key={idx}>{weapon.name} ({weapon.type}) - Kills: {weapon.kills}</li>
                ))}
              </ul>
            </div>
          </div>
        ...
        {activeHobby.name === "Misc" && activeHobby.data && (
          <div className="bg-[#111] p-4 rounded-md text-green-300 text-xs space-y-2 mb-4">
            <p>Other interests:</p>
            <ul className="list-disc pl-6 space-y-1">
              {activeHobby.data.map((item, idx) => <li key={idx}>{item}</li>)}
            </ul>
          </div>
        )}

        {/* Render component if exists */}
        {activeHobby.component && activeHobby.name !== "Projects" && <activeHobby.component />}
      </div>
    </section>
  );
}
