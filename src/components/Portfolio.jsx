    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white font-sans px-8 py-16 flex flex-col items-start space-y-24 max-w-5xl mx-auto">
      ...
      <section className="space-y-4">
        ...
        <p className="text-white/70 text-lg md:text-xl">
          Frontend Engineer at Sequoia | Formerly Leadsquared & SAP Labs | MTech BITS Pilani | Passionate Web Developer
        </p>
      ...
      {/* Summary */}
      <section className="space-y-4">
        <h2 className="text-3xl font-bold text-white drop-shadow-[0_0_10px_rgba(0,255,0,0.7)]">Summary</h2>
        <p className="text-white/70 leading-relaxed text-sm">
          Frontend Engineer at Sequoia with 6.5+ years of experience in building scalable web apps,
          improving UX, implementing CI/CD pipelines, and enhancing system security. Delivered high-impact projects like Manage Activities Tab, Calendar & Recurring Task, and performance/security improvements.
          Passionate about creating user-centric web solutions and continually growing as a software engineer.
        </p>
      </section>
        ...
        <div className="relative border-l-2 border-green-600 pl-8 space-y-12">
          {EXPERIENCE.map((job, idx) => (
            <div key={idx} className="relative">
              ...
              <h3 className="font-bold text-white text-lg drop-shadow-[0_0_5px_rgba(0,255,0,0.7)]">{job.role}</h3>
              <p className="text-white/70 text-sm">{job.duration} | {job.location}</p>
              <p className="text-white/60 mt-2 whitespace-pre-line">{job.description}</p>
              {job.skills && (
                <div className="mt-1 flex flex-wrap gap-2 text-xs text-green-400">
                  {job.skills.map((s, i) => (
                    <span key={i} className="px-2 py-0.5 border border-green-500 rounded-md">{s}</span>
                  ))}
                </div>
              )}
            </div>
      ...
      <section id="projects" className="space-y-6">
        <h2 className="text-3xl font-bold text-white drop-shadow-[0_0_10px_rgba(0,255,156,0.6)] mb-6">Featured Projects</h2>
          ...
          {PROJECTS.map((p, i) => (
            ...
            >
              ...
              <div className="p-5 flex flex-col justify-between gap-3">
                <div>
                  <h3 className="text-xl font-semibold text-white drop-shadow-[0_0_6px_rgba(0,255,156,0.6)]">{p.title}</h3>
      ...
      {/* Certifications */}
      <section className="space-y-4">
        <h2 className="text-3xl font-bold text-white drop-shadow-[0_0_10px_rgba(0,255,0,0.7)]">Certifications</h2>
        <ul className="list-disc pl-6 text-white/70 space-y-1">
          {CERTIFICATIONS.map((c, i) => (
            <li key={i}>
              {c.name} - <span className="text-green-400">{c.issuer}</span> ({c.date})
            </li>
          ))}
        </ul>
      </section>
      ...
      {/* Awards */}
      <section className="space-y-4">
        <h2 className="text-3xl font-bold text-white drop-shadow-[0_0_10px_rgba(0,255,0,0.7)]">Recognitions & Awards</h2>
        <ul className="list-disc pl-6 text-white/70 space-y-1">
          {AWARDS.map((a, i) => (
            <li key={i}>
              {a.year} - {a.title}
